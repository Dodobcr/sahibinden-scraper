...
DELAY_MIN = float(os.environ.get("DELAY_MIN", "4"))
...
def run_scrape(driver, urls: list, seen: set) -> tuple:
    ...
    for u_idx, base_url in enumerate(urls):
        ...
        while page_num <= MAX_PAGES:
            ...
            print(f"  {len(links)} ilan", flush=True)
            ...
            for i, det_url in enumerate(links):
                ...
                print(f"  [{i+1}/{len(links)}] #{ilan_id} – {wait:.1f}s...", flush=True)
                time.sleep(wait)
                ...
                try:
                    ...
                    save_seen(seen)
                    total_new += 1
                    print(f"  ✓ #{row.get('İlan No',ilan_id)} | "
                          f"{row.get('Fiyat','?')} | "
                          f"{row.get('İlçe','?')} | "
